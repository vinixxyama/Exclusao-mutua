...
def sender():
    while True:
        ...
        if(int(opcao) == 1):
            ...
            mensagem = str(config.clock) + "/" + str(pid) + "/" + str(message) + "/" + str(msgPID)
            ...
            ACKmsg = str(config.clock) + "/" + str(pid) + "/" + "ACK" + "/" + str(msgPID)
            sock.sendto(ACKmsg, ("224.0.0.251", 1050))

            # Input the message
            message = raw_input("Type in your message: ")
            print("msgPID: " + str(msgPID))
    ...
    while True:
        ...
        if(msgsplit[2] != "ACK"):
            ...
            for aux in range(1, processcount + 1):
                # If its known
                if(ACK[aux][0] == msgsplit[3]):
                    break

                # If not known, add it to the matrix
                else:
                    if(ACK[aux][0] == 0):
                        ACK[aux][0] = msgsplit[3]
                        break

            # Create the tuple to be queued (clock, PID)
            msg = (msgsplit[0], msgsplit[1], msgsplit[3])
            listaInicial.append(msg)

            # Create the ACK message to multicast
            ACKmsg = str(config.clock) + "/" + str(pid) + "/" + "ACK/" + str(msgsplit[3]) 
            sock.sendto(ACKmsg, ("224.0.0.251", 1050))
        ...
        # If it is an ACK
            ...
            if(recebido == 1):
                ...
                for aux in range(0, processcount + 1):
                    for aux2 in range(0, processcount + 1):
                        ACK[aux][aux2] = 0
                
                retorno = listaInicial[config.conta]
                config.conta = config.conta+1
                
                # Insert into FinalQueue for verification
                FinalQueue.put(retorno)
                print("\nMensagem do processo com PID: " + str(retorno[1]) + " e msgPID: " + str(retorno[2])
                       + " processada\n")
